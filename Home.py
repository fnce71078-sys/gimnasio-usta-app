...
if opcion == "📲 Registrar Asistencia":
    ...
    # clear_on_submit=True limpia la caja de texto tras registrar para que el siguiente estudiante lo encuentre vacío
        ...
        if btn_ingreso:
            ...
            if not codigo_clean:
                ...
                with st.spinner("Validando credenciales en el sistema..."):
                    ...
                    if db is not None:
                        try:
                            # Nos conectamos a ambas hojas de la base de datos
                            sheet_usuarios = db.worksheet("Usuarios")
                            sheet_asistencias = db.worksheet("Asistencias")
                            
                            # Traemos todos los datos de Usuarios de un solo golpe para no gastar cuota de API
                            datos_usuarios = sheet_usuarios.get_all_values()
                            
                            # Extraemos solo los códigos de la primera columna (saltando el encabezado)
                            ...
                            if codigo_clean not in codigos_existentes:
                                ...
                                # Magia: Buscamos el nombre exacto de ese código
                                nombre_usuario = "Usuario" # Por defecto
                                for fila in datos_usuarios[1:]:
                                    if len(fila) >= 2 and fila[0] == codigo_clean:
                                        nombre_usuario = fila[1] # La columna 2 es el Nombre Completo
                                        break
                                
                                # Capturamos la fecha y hora en columnas separadas
                                ahora = datetime.now()
                                fecha_actual = ahora.strftime("%Y-%m-%d")
                                hora_actual = ahora.strftime("%H:%M:%S")
                                
                                # Inyectamos silenciosamente en la pestaña Asistencias (Código, Fecha, Hora)
...
# ---------------------------------------------------------
    ...
    # Vectores de opciones estandarizadas
    ...
    # Bloque interactivo fuera del formulario para dinamismo en tiempo real
    ...
    with st.form("formulario_registro", clear_on_submit=False):
        ...
        # Ocultación de campos según el Rol para simplificar la UI en móviles
        ...
        if btn_enviar:
            ...
            # Homologación automática de variables de entorno para el Excel
            ...
            if not nombre_c or not codigo_c or not contacto_c or not tel_c:
                ...
                # Apertura del canal de datos e inspección de duplicados en la llave primaria
                    ...
                    if db is not None:
                        try:
                            ...
                            if codigo_c in codigos_existentes:
                                ...
                                # Creación del Timestamp automatizado
                                fecha_registro = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                
                                # Matriz ordenada de datos alineada con Google Sheets
                                nueva_fila = [
                                    codigo_c,
                                    nombre_c,
                                    rol_seleccionado,
                                ...
                                # Inyección ordenada de la fila en Google Sheets
                                sheet_usuarios.append_row(nueva_fila)
                                
                                # Mensaje de éxito estructurado e instructivo
